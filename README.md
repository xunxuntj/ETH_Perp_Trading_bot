...
- 🤖 **灵活部署** - 支持 GitHub Actions（兜底）/ 外部 Cron（推荐）/ VPS 部署
...
│  生存期 (30m ST 托管)                            │
│  ├─ 止损: 跟随 30m ST，只紧不松                  │
│  └─ 触发锁利: 按 30m ST 平仓收益 >= Buffer       │
├─────────────────────────────────────────────────┤
│  锁利期 (跟随 1H ST，只紧不松)                   │
│  ├─ 止损: max(30m ST, 1H ST) ← 取更紧的         │
│  └─ 触发换轨: 1H ST 比锁利阈值更紧               │
├─────────────────────────────────────────────────┤
│  换轨期 (1H ST 托管)                             │
│  ├─ 止损: 跟随 1H ST，只紧不松                   │
...
**冷静期规则:**
- 连续 3 笔亏损 → 强制休息 48 小时
- 自动交易模式: 每次平仓后增量记录连续亏损，冷静期结束后计数清零并重新开始
- 信号模式: 基于交易所平仓历史统计连续亏损，冷静期结束后通过 reset_anchor_time 清零历史影响
- 本金 ≤ 350U → 停手 1 周
...
> ⚠️ **重要提示**：GitHub Actions 的 `schedule` 触发器**不可靠**！
> - 实际运行频率远低于配置（配置每 5 分钟，实际可能每小时才运行 1 次）
> - 有延迟（几分钟到几小时）
> - 高峰时段可能跳过执行
> 
> **对于需要精确定时的交易机器人，强烈建议**：
> - 使用 [外部 Cron 服务](EXTERNAL_SCHEDULING_GUIDE.md)（免费，更可靠）
> - 或部署到 [VPS/云服务](DEPLOYMENT.md#方案-2️⃣-vps-服务器)
> 
> GitHub Actions 仅适合作为**兜底备份**，不建议作为主要运行方式。
