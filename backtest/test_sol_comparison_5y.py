#!/usr/bin/env python3
"""
SOL current strategy vs Planned portfolio comparison (5-Year):
1. SOL_USDT - Current v9.7 (No TP, 2.0% Risk)
2. SOL_USDT - Planned Strategy (5.0R TP, 2.0% Risk)
3. BTC_USDT + ETH_USDT Portfolio - Planned Strategy (BTC 22.0R, ETH 5.0R, 2.0% Risk)
"""
...
class SimulationEngine:
    ...
    def run_simulation(self, datasets: dict, configs: dict, risk_percent: float) -> dict:
        """Runs joint portfolio simulation."""
            ...
            for c in contracts:
                if in_pos[c]:
                    ...
                    if is_stopped or (is_tp_hit and exit_reason == "take_profit"):
                        ...
                        # Technical exit if SuperTrend flips
                        tech_exit = (phase[c] == 3 and h1_st_dirs[c][i-1] == -1 if is_long else phase[c] == 3 and h1_st_dirs[c][i-1] == 1) or \
                                    ((phase[c] == 1 or phase[c] == 2) and st_dirs[c][i-1] == -1 if is_long else (phase[c] == 1 or phase[c] == 2) and st_dirs[c][i-1] == 1)
                        
                        if tech_exit:
                            exit_price = opens[c][i]
                            actual_exit = exit_price - slippages[c] if is_long else exit_price + slippages[c]
                            pnl = (actual_exit - entry_price[c]) * qty[c] * face_values[c] if is_long else (entry_price[c] - actual_exit) * qty[c] * face_values[c]
                            fee = (entry_price[c] + actual_exit) * qty[c] * face_values[c] * self.fee_rate
                            net_pnl = pnl - fee
...
def run_sol_comparison():
    contracts = ["BTC_USDT", "ETH_USDT", "SOL_USDT"]
    datasets = {}
    
    print("=" * 80)
    print("🚀 PRECOMPUTING INDICATORS FOR COMPARISONS (5-YEAR)")
    print("=" * 80)
    
    # Common indicator logic
    for contract in contracts:
    ...
    print("\n" + "=" * 80)
    print("📈 RUNNING 5-YEAR STRATEGY PATH COMPARISONS (2.0% RISK)")
    print("=" * 80)
    
    # 1. Run SOL Current (v9.7) - No TP
    sol_cur_configs = {
        "SOL_USDT": {"trend_filter_1h": "ema", "dema_period": 200, "adx_threshold": 30.0, "tp_ratio": None}
    }
    sol_cur_datasets = {"SOL_USDT": datasets["SOL_USDT"]}
    res_sol_cur = sim.run_simulation(sol_cur_datasets, sol_cur_configs, risk_percent=0.02)
    
    # 2. Run SOL Planned (with 5R TP)
    sol_tp_configs = {
        "SOL_USDT": {"trend_filter_1h": "ema", "dema_period": 200, "adx_threshold": 30.0, "tp_ratio": 5.0}
    }
    res_sol_tp = sim.run_simulation(sol_cur_datasets, sol_tp_configs, risk_percent=0.02)
    
    # 3. Run Recommended BTC + ETH Portfolio (BTC 22R, ETH 5R)
    portfolio_configs = {
        "BTC_USDT": {"trend_filter_1h": "ema", "dema_period": 200, "adx_threshold": 25.0, "tp_ratio": 22.0},
        "ETH_USDT": {"trend_filter_1h": "ema", "dema_period": 200, "adx_threshold": 30.0, "tp_ratio": 5.0}
    }
    portfolio_datasets = {
        "BTC_USDT": datasets["BTC_USDT"],
        "ETH_USDT": datasets["ETH_USDT"]
    }
    res_portfolio = sim.run_simulation(portfolio_datasets, portfolio_configs, risk_percent=0.02)
    
    # Print comparison
    print(f"\n📊 COMPARATIVE RESULTS:")
    print(f"{'Metric':22s} | {'1. SOL Current (v9.7)':22s} | {'2. SOL + 5R TP':22s} | {'3. BTC+ETH Portfolio':22s}")
    print("-" * 95)
    print(f"{'5-Year CAGR':22s} | {res_sol_cur['cagr']*100:+21.2f}% | {res_sol_tp['cagr']*100:+21.2f}% | {res_portfolio['cagr']*100:+21.2f}%")
    print(f"{'Max Drawdown':22s} | {abs(res_sol_cur['max_dd'])*100:21.2f}% | {abs(res_sol_tp['max_dd'])*100:21.2f}% | {abs(res_portfolio['max_dd'])*100:21.2f}%")
    print(f"{'Profit Factor (PF)':22s} | {res_sol_cur['profit_factor']:21.2f}  | {res_sol_tp['profit_factor']:21.2f}  | {res_portfolio['profit_factor']:21.2f} ")
    print(f"{'Win Rate':22s} | {res_sol_cur['win_rate']*100:21.2f}% | {res_sol_tp['win_rate']*100:21.2f}% | {res_portfolio['win_rate']*100:21.2f}%")
    print(f"{'Total Trades':22s} | {res_sol_cur['total_trades']:21d}  | {res_sol_tp['total_trades']:21d}  | {res_portfolio['total_trades']:21d} ")
    print(f"{'Final Equity':22s} | ${res_sol_cur['final_equity']:20.2f}  | ${res_sol_tp['final_equity']:20.2f}  | ${res_portfolio['final_equity']:20.2f} ")
    print("=" * 95)
