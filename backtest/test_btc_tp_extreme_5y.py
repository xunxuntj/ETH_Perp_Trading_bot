#!/usr/bin/env python3
"""
BTC Extreme Take Profit (TP) Sweep (5-Year):
Sweeps TP ratios in [10.0, 12.0, 15.0, 20.0, 9999.0] for BTC_USDT
...
from indicators import (
    calculate_supertrend, calculate_ema, calculate_dema, calculate_atr, calculate_adx
)
...
class TPSweepSimulator:
    ...
    def get_contract_params(self, contract: str) -> tuple:
        return 0.0001, 1.0  # BTC params

    def run_btc(self, df: pd.DataFrame, tp_ratio: float, risk_percent: float) -> dict:
        n = len(df)
        ...
        face_value, slippage = self.get_contract_params("BTC_USDT")
...
def run_tp_sweep():
    df_30m, df_1h = load_data("BTC_USDT")
    if df_30m is None or df_1h is None:
        print("❌ Missing data files. Run download first.")
        return
    ...
    print("\n" + "=" * 80)
    print("📈 RUNNING EXTREME TAKE-PROFIT SWEEPS FOR BTC_USDT (5-YEAR, 2.0% RISK)")
    print("=" * 80)
    
    tp_values = [14.0, 16.0, 18.0, 20.0, 22.0, 24.0, 26.0, 28.0, 30.0] # Finer sweep for BTC
    
    print(f"{'TP Ratio':10s} | {'5-Year CAGR':12s} | {'Max Drawdown':12s} | {'Profit Factor':13s} | {'Win Rate':10s} | {'Final Equity':12s}")
    print("-" * 75)
    
    for tp in tp_values:
        res = sim.run_btc(df_merged, tp, risk_percent=0.02)
        label = f"{tp:.1f}R" if tp < 9000 else "No TP (Uncapped)"
        print(f"{label:10s} | {res['cagr']*100:+11.2f}% | {abs(res['max_dd'])*100:11.2f}% | {res['profit_factor']:13.2f} | {res['win_rate']*100:9.2f}% | ${res['final_equity']:10.2f}")
    print("=" * 80)
